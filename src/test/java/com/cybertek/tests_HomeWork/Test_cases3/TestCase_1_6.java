...
public class TestCase_1_6 {
    ...
    public void test6() throws InterruptedException {
        ...
        WebElement checkBoxClick = driver.findElement(By.xpath("(//tr[@class='grid-row row-click-action'])[14]"));
        Thread.sleep(5000);
        checkBoxClick.click();

        List<WebElement> information = driver.findElements(By.xpath("//div[@class='control-group attribute-row']"));

        for (WebElement dataInfo : information) {

            System.out.println(dataInfo.getText());
        }

        for (int i = 1; i <= 10; i++) {
            String xpath="//div[@class='control-group attribute-row']["+i+"]";
            WebElement elements=driver.findElement(By.xpath(xpath));

            if(elements.isDisplayed()==true){
                System.out.println(elements+ " "+ i + " "+ "value is display");
            }else{

                System.out.println(elements+ " "+ i + " "+ "value is NOT display");
